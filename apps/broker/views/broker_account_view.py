...
from apps.common.security.encryption import encrypt_api_key, hash_api_key

logger = logging.getLogger(__name__)


# <<--------------------------------- Create Broker Account View --------------------------------->>
@allowed_methods("POST")
class CreateBrokerAccountView(APIView):
    ...
    def post(self, request):
        try:
            ...
            # Prevent duplicate broker account per user
            if self.model_class.objects.filter(user=request.user).exists():
                data = {"message": "Broker account already exists", "error": "Broker account already exists"}
                return response_formatter(BAD_REQUEST_USER_ERROR, data)

            with atomic():
                validated_data = serializer.validated_data
                raw_api_key = validated_data.get("api_key")
                validated_data["user"] = request.user
                # Create new user instance
                instance = create_instance(self.model_class, serializer.validated_data, extract_fields=["api_key"])
                instance.api_key = encrypt_api_key(raw_api_key)
                instance.api_key_hash = hash_api_key(raw_api_key)
                instance.save()

                # Minimal response payload to reduce response size
                data = {"id": instance.id, "message": "Created Broker Account"}
                return response_formatter(SUCCESS_RESPONSE_201, data)

        except Exception as e:
            logger.exception(f"ERROR:---------->>Create Broker Account View error: {e}")
